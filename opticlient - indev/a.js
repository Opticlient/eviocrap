...
//benchmark performance optimizations (DO NOT DISTRIBUTE!!! (removed for causing problems))
(() => {
 return;
 window.addEventListener("load", () => setTimeout(() => {
...
//optimize fps (DO NOT DISTRIBUTE!!! (it feels weird somehow))
(() => {
 return;
 let list = [],
 active = false,
 ...
 loop = time => {
  if (overloaded) {
   requestIdleCallback(_loop, timeout);
   console.log2(true);
  } else {
   _requestAnimationFrame(loop);
  };
  active = true;
  for (let item, index = 0, length = list.length; index < length; index++) {
   item = list[index];
   if (item) {
    delete list[index];
    try {
...
//fixed forced high quality weapon bug (DO NOT DISTRIBUTE!!! (this doesn't work))
(() => {
 return;
 const _fetch = window.fetch;
 window.fetch = function(url, options) {
  if (url?.endsWith?.("/me?_format=json")) {
   return new Promise(resolve => {
    _fetch(url).then(e => e.json()).then(_e => {
     _e.forEach(e => {
      [
       [ "auto_rifle", 4 ],
       [ "burst_rifle", 336 ],
       ...
       i[0] = "field_" + i[0] + "_skin";
       console.log2(e[i[0]][0]);
       if (e[i[0]]?.[0]?.target_id === i[1]) {
        e[i[0]].pop();
        console.log2(true);
       };
      });
     });
     _fetch("data:," + encodeURIComponent(JSON.stringify(_e))).then(e => {
      window.fetch = _fetch;
      resolve(e);
     });
    });
